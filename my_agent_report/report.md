...
**Review period:** The intended scope is a one-year retrospective, with exact boundaries still to be confirmed. The current account starts with my arrival at miHoYo in November 2025, covers Echo's emergence after the OpenClaw surge I recall in January 2026 and the following two to three months, and includes infrastructure development in March–April 2026 and a founder's April push for Project S. Echo's precise dates, the expansion period's boundaries, and the dates of Project S's rollouts and outcomes remain open. The ByteDance comparison covers late 2023 through the end of 2025; Anuttacon provides a comparison within the Project S case.
...
**Unresolved permissions:** I considered the permissions model inadequately designed. The question of which internal systems personal agents and team agents should be allowed to access remained unresolved throughout the period I observed. The meaning of each agent category, concrete permissions questions, and the observation's end date remain to be specified. C07 provides a specific case involving access across a team's data and shared memory.
...
**Where to invest development effort:** The sequence moves from shared-platform discussion, through a separate team's client product, to aggressive framework reuse and employee-written skills, followed by widespread discontinuation of departmental deployments. C04–C07 provide a basis for examining which shared capabilities and company-level responsibilities are needed to sustain employee-created agents. The causes of individual shutdowns and effects on the December proposal and Echo's roadmap remain open. Timing alone does not establish that Echo caused every subsequent decision.
...
The assessment should distinguish infrastructure availability, application breadth, actual use, workflow completion, and cost relative to results. C03 records Echo's adoption success in my assessment; C04 now spans early adoption and widespread discontinuation. Business value still needs evidence, as do the specific reasons for discontinuation. C06 adds interface usability, context consumption, and access permissions; C07 adds the fit between a team agent's design and the organization's permissions and work practices.
