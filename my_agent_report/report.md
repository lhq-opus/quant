...
Project S was useful when a colleague needed a data warehouse table design. During roughly two months adapting colleagues' workflows to it, I found that connecting requirements, instrumentation, returned data, table design, and validation demanded much more. Agent permissions were not properly designed, team memory lacked isolation, and some stages had no clear acceptance criteria. These are the concrete limits behind my concern about moving from individual productivity to end-to-end delivery.
...
**My implementation work.** While on the data team, I spent roughly two months carrying out forward-deployed engineer (FDE) responsibilities, adapting colleagues' workflows to Project S. The exact dates of this work remain to be confirmed. The full data workflow I worked to support covered:
...
**My reflection from adapting the data workflow:** Connecting the steps required the agent to repeatedly revise and verify its work, placing very high demands on the harness and agent infrastructure. A separate difficulty was that **not every stage had clear, quantifiable acceptance criteria**. In my assessment, when individual tasks were joined into an end-to-end workflow, ambiguity at any such stage could cause the agent to drift away from the original task objective.

**Why a good table design did not establish end-to-end delivery — working analysis.** Table design is the fifth step in the six-step workflow above. Its quality alone cannot establish that the requirement was understood, the right data returned, or the final result answered the business question. These are acceptance questions for my actual workflow, not a reconstruction of a recorded failed run:

| Stage in my workflow | What must be established before accepting the result |
| --- | --- |
| Understand the requirement; collect its context | The objective, scope, and business definitions are agreed, and relevant material is available with appropriate access. |
| Design instrumentation; receive returned data | There is actual returned data to check against the instrumentation plan. Producing the plan alone does not complete these steps. |
| Design the warehouse table | The table fits the data actually received and preserves the business meaning established earlier. |
| Run processing and validate | The processing runs, and its output answers the original requirement. A responsible person reviews any part requiring business judgment. |

Two different problems appear here. The MCP pagination burden in Case Study 4 consumes context during data collection; unresolved access and memory boundaries constrain which context can be used. Separately, a step can produce an output without establishing that it still serves the original requirement. More workers or researchers do not supply a missing business definition or acceptance decision. My account identifies these constraints and the risk of goal drift; it does not yet include a trace of a particular drift incident.
...
*Evidence: U06, U07, S05, S11. The acceptance table is analysis of my workflow. S05 supplies organizational context; S11 supplies an external harness example.*
...
**Working analysis of the shift:** Client adoption and warehouse table design had shown useful results. The remaining problems were specific: MCP data retrieval required repeated pagination; employee-like agents lacked defined authority; team memory lacked isolation; and a completed table design still left the rest of the workflow to validate. Extending mature frameworks to address those problems had a concrete purpose. My roughly two months adapting colleagues' workflows also show the need for someone to identify the actual systems, context, and acceptance decisions a particular workflow requires. That helps explain the demand for FDE experience. It is a case-based explanation, not a documented company statement of why it changed course.
...
| Leverage | Work with users, locate a consequential bottleneck, and test a small change. | I spent roughly two months adapting colleagues' full workflows. Table design was one step; my reflection concerned repeated verification across steps and unclear acceptance criteria. |
...
*Evidence: U11, D01, S06–S10. The principle-to-case mapping and explanation of the two data cycles are working analysis. Anuttacon's FDE requirement and miHoYo's move toward harness engineering are different decisions in different companies.*
...
| Project S produced useful table designs within a longer data workflow. | Fund one full workflow with a named business owner and an FDE responsible for adaptation and handoffs. Agree on the acceptance questions in Case Study 5 before the run. | A trace from the original requirement to accepted data results, including changes in interpretation, retries, human intervention, and total cost. |
...
| Anuttacon recognized a need for FDE while promoting its models. | If our model rollout also needs customer adaptation, assign responsibility for an accepted customer result. If model training is intended, retain evaluated task outcomes and distinguish model errors from tool, access, or context defects. | A used and accepted workflow first; then evidence that feedback improves performance on a separate task or customer. A collected conversation alone is insufficient. |
