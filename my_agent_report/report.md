...
| With long contexts, the model did not follow skills consistently. | Assign deterministic procedures to hooks, usually maintained through scripts. | A subagent stop hook can run data-quality scripts; the specific checks and handling of failures remain to be documented. |
...
**A concrete point for running checks.** A subagent stop hook can run data-quality scripts. Some scripts come from testing; others come from the workflow's ongoing self-iteration. This illustrates how we connected deterministic checks to agent execution. How failed checks feed back into revision, retries, or termination, and how scripts produced through iteration enter the checks, still need to be documented.
...
| U14 | My account of deterministic procedures, stop hooks, and the sources of check scripts | Recorded September 17, 2026 | Deterministic procedures are assigned to hooks and usually maintained through scripts. A subagent stop hook can run data-quality scripts, sourced from testing and the full workflow's ongoing self-iteration. This identifies a concrete execution mechanism; particular check results, failure feedback, retry/termination behavior, script production and integration, and measured effects remain undocumented. It does not establish that the hook blocks exit or that agents may change business criteria themselves. |
