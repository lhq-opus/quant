...
- `2`：普通限价单。读取 CSV `Price`，买单逐档成交至该价格为止，卖单反向处理；
  未成交数量以 `Price` 加入本方盘口；
- `1`：即时成交剩余撤销（IOC）。不使用 CSV `Price` 作为限价；买单从卖一继续吃
  卖二、卖三，卖单从买一继续吃买二、买三，不限五档。数量全部成交或对手盘耗尽时
  停止，未成交部分直接撤销、不加入本方盘口；对手盘为空时全部撤销；
...
每条 order 都用 `(ChannelNo, OrderApplSeqNum)` 记录实际挂单价格，或用 0 标记不留挂单。
例如 `U` 买单原始 `Price=0`，到达时买一是 10 元，那么记录的价格就是 10 元。之后
该单撤销 30 股时，按原订单引用查到 10 元，再从买盘该档扣除 30 股；不使用撤单行
保留的原始零价格查盘口。

类型 `1` 始终不留下挂单，类型 `U` 在本方盘口为空时也不入簿，索引均记为 0。后续对应的
...
- 买单从最低卖价开始，卖单从最高买价开始，每档成交量取来单剩余量和该档数量的较小值；
- 每次删掉吃空的档位后，下一档成为新的最优档，继续循环；各档按被消耗的对手方价格成交；
- 只对类型 `2` 检查价格边界：对手卖价不能高于买入限价，对手买价不能低于卖出限价；
- 循环结束后，只对类型 `2` 把余量挂入自己的限价档；类型 `1` 的余量撤销。

例如卖一 `10.00×100`、卖二 `10.01×200`，到达一笔类型 `1` 买单 150 股：先在
10.00 成交 100，再在 10.01 成交 50；最后卖一变为 `10.01×150`，成交额为 1500.50。
若买单改为 350 股，则两档共成交 300，剩余 50 直接撤销，不形成买盘。

类型 `U` 不需要撮合，直接在本方最优档增加数量。上述类型 `1` 是本 demo 选择的
IOC 子类型，不是“对手方最优、剩余转限价”，也不是“最优五档剩余撤销”。
成交部分仍只操作“价格 → 聚合数量”，所以能重建价格档、
