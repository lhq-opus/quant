...
`ExecType=4` 的 trade，并把撤单引用的原订单方向和价格分别写入撤单行的 `Side`、
`TradePrice`，最后按到达时间生成 `event.csv`。
...
4. 用该 order 的原始 `Side` 和 `Price` 补全撤单事件。
...
- 撤单 trade 行保留 `caa,Side,ExecType,TradeQty,TradePrice`，其中 `Side` 和
  `TradePrice` 来自被撤原订单，其余 order 专有列为空；
