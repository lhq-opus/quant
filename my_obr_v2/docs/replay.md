...
存在暂存单时也允许新限价提前扣盘，解冻单的成交和 last price 仍归当前委托快照。
...
- `record_trade`：仅对真实 F 调用一次，更新笔数、累计量额、最新价；
  apply 中的开盘竞价 F 同时确定开盘价。CYB 缓存的 F 延迟到回放时累计，
  其余 F 在 apply 中累计；撤单、限价推演和市价回放不再增加成交统计。
...
如果没有待处理的 CYB 组，立刻将原委托快照设为 Ready；未成交限价可在来单时直接 Ready。
有暂存单时，新限价同样先完成自身撮合，但快照仍等待解冻单的真实成交归入该组。
...
卖单处理对称。因此合格新限价沿用现有 while，先完成自身预测扣盘及余量入簿，
随后复用空回放恢复符合范围的暂存余量，不再因为暂存表非空而跳过新单撮合。

当前新单的真实 F 按原单序号及待确认量识别，只确认预测量和累计真实统计；
即使该单已经全成并删除索引，也不会再次解引用原单或扣盘。解冻单的 F 仍按真实
双方引用在 CYB 回放中执行。pending_cyb_group 仅让当前快照等到组结束，
使解冻单的成交、累计量额和最后成交价也属于当前新单的 snapshot，元信息仍取新单。
handle_pending_CYB_limit_order 保留原有缓存入口，已预测 F 不进入缓存，
市价关联 F 优先由市价回放处理，其余组内 F 只进入一份缓存。
同一 F 即使两侧都为暂存单，或一侧暂存、一侧市价，也只执行和统计一次。

用户在 `234c0b` 实现并再次确认的接收特性是：撤单删除买一/卖一，触发暂存单后，
先收到这组真实成交 `F1、F2`，最后才收到 `Cancel`。处理时保留这个接收顺序：

- 当前新限价已预测的 F 只确认和统计；其余 CYB F 到达时进原缓存，盘口和统计
  延迟到回放，避免后到撤单触发的成交提前污染上一张快照。
- Cancel 到达后，逐笔回放可能属于上一张委托的普通成交前缀。遇到暂存买单成交的
  对手价不同于当前卖一，且当前 Cancel 恰好会删掉整个卖一档时，从这笔 F 开始
  标记 `replay_caused_by_cancel`；暂存卖单对买一的判断对称。
  部分撤单、同价仍有其他原单，以及正常委托使最优档变化后的成交不满足该条件。
- 每次判断前复用空缓存回放，恢复此前普通成交或市价成交已经激活的其他暂存单，
