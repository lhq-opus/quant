...
以下修复前行号均对应 bb37942。本轮又发现两类缺陷，之前的 32 个场景未覆盖它们。
...
### 2. 已修复：旧未定价市价余量导致限价误撮合、负余量及错误快照

当前 v2 已明确支持“没有成交价的市价余量暂不入簿，后来收到真实 F 再定价”。
但 `apply_limit_order:216–241` 原来仍只看可见盘口提前撮合，忽略了这一份未定价余量。
因此来单预测的对手、成交量可能与随后真实 F 不一致。
以下均是该 v2 状态契约的合成验证，不声称是完整的交易所行情轨迹。

**负余量及崩溃复现：**

| 顺序 | 输入 | 按实际成交应有的状态 |
| --- | --- | --- |
| 1 | 市价买 #1，100 股，没有 F | #1 未定价，不进入可见买盘 |
| 2 | 限价买 #2，10 元 × 50 股 | #2 可见，余量 50 |
| 3 | 限价卖 #3，10 元 × 100 股 | 本组随后 F 确认实际对手为 #1 |
| 4 | F：买 #1、卖 #3，10 元 × 100 股 | #1 和 #3 全成；#2 仍有 50 |
| 5 | 撤买 #2 的 50 股 | 删除 #2，双侧为空 |

修复前，第 3 步提前吃掉 #2，并把 #3 的“预测余量”50 入簿。
第 4 步识别为旧市价成交，回放又从 #3 扣 100，使卖档余量变成 **-50**；
第 5 步撤销实际上未成交的 #2 时，索引已经被错误删除，再次访问无效 position。
已在 ASan 下复现 SEGV。该过程每个订单的关联 F 都在下一张委托/撤单前到齐，
与下节的 CAA 错序问题不同。

**静默错误复现：** 删除上述 #2，仅保留“市价买 #1 → 限价卖 #3 → F(#1,#3)”。
#3 到达时看不到买盘，预测量为零，因此立即输出卖量 100、nts=0 的 Ready 快照。
F 到达后内部盘口虽然被扣空，快照已经输出；最终缺少正确的卖量 0、nts=1 状态。
旧代码 `update_previous_snapshot` 只更新 Pending，无法补救提前输出的行。

**本次处理：** 增加两个私有状态，业务方法仍保持原名：

- `unpriced_market_orders` 只记录旧未定价市价原单号，不包含等待自动撤销的空本方 U 单。
  有价入簿、全撤或恢复当前市价回放时同步移除，避免每个来单遍历全部活跃订单。
- `pending_unpriced_group` 表示本组可能涉及这类余量。该组限价先通过原入簿方法登记
  完整数量，真实 F 再沿原 `execute_trade` / 市价缓存扣双方，跳过提前撮合。
  原快照等待下一张委托、撤单、阶段切换或 EOF 后完成，保留其自身元信息。

没有旧未定价市价余量时，普通限价仍按原逻辑提前撮合，并在相关 F 确认齐后立即输出。
这些改动处理业务状态，不添加输入格式检查，也不以“找不到订单就忽略”掩盖漏扣。
...
前两项缺陷的复现和修复均不依赖相同 CAA；完整 CLI 修复验证使用严格递增 CAA。
尚未取得实际 core 堆栈，因此只能确认这两类代码缺陷已复现，不能声称已确定用户那次
coredump 的唯一根因。
