...
void open_inputs(const std::string& order_path, const std::string& trade_path) {
  // 重放文件再次打开时先结束上一轮流状态；EOF 是正常业务结束，不做格式猜测。
...
CommandLineOptions parse_command_line(int argc, char* argv[]) {
  // 保留用户现有三个路径参数的位置约定，输入合法，无需使用 argc 做格式检查。
  (void)argc;
...
std::string format_fixed_point(int64_t value) {
  // 保留两位小数输出，在整数域四舍五入到分，避免大额转 double 丢失低位。
  // 先求无符号幅值，不对最小有符号整数直接取负。
...
void write_book_row(std::ofstream& output, Snapshot& snapshot) {
