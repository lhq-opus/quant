...
void OrderBook::execute_auction_trade(int64_t price, int64_t quantity)
{
    ...
    while (remaining_quantity > 0)
    {
        ...
        BookLevel &bid_level = bid->second;
        RestingOrder &bid_order = bid_level.orders.front();
        bid_order.remaining_quantity -= traded_quantity;
        bid_level.total_quantity -= traded_quantity;
        if (bid_order.remaining_quantity == 0)
        {
            order_info_map.erase(bid_order.order_appl_seq_num);
        ...
        BookLevel &ask_level = ask->second;
        RestingOrder &ask_order = ask_level.orders.front();
        ask_order.remaining_quantity -= traded_quantity;
        ask_level.total_quantity -= traded_quantity;
        if (ask_order.remaining_quantity == 0)
        {
            order_info_map.erase(ask_order.order_appl_seq_num);
